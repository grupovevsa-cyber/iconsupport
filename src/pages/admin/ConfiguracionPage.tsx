...
import {
  ...
  ChevronDown, ChevronUp, Info, Building
} from 'lucide-react'
...
export function ConfiguracionPage() {
  ...
  // Cargar lista de empresas (para superadmin) o determinar empresa_id del admin
  useEffect(() => {
    const init = async () => {
      setLoading(true)
      if (isSuperAdmin) {
        const { data } = await supabase.from('empresas_saas').select('id, nombre').order('nombre')
        if (data && data.length > 0) {
          setEmpresas(data)
          setSelectedEmpresaId(data[0].id)
        }
      } else if (user?.profile?.empresa_id) {
        setSelectedEmpresaId(user.profile.empresa_id)
      } else {
        setLoading(false) // No tiene empresa (ej: cliente o sin asignar)
      }
    }
  ...
  return (
    <div className="min-h-screen bg-surface-950 pb-20">
      ...
      <div className="max-w-3xl mx-auto px-4 py-6 space-y-4">
        ...
        {isSuperAdmin && (
          <div className="bg-surface-900 border border-slate-800 rounded-2xl p-5 mb-6 flex flex-col sm:flex-row sm:items-center gap-4">
            <div className="flex-1">
              <h3 className="text-sm font-bold text-white flex items-center gap-2">
                <Building size={16} className="text-brand-400" /> Seleccionar Empresa
              </h3>
              <p className="text-xs text-slate-400 mt-1">Estás editando la configuración de:</p>
            </div>
            <select
              value={selectedEmpresaId || ''}
              onChange={(e) => setSelectedEmpresaId(e.target.value)}
              className="bg-surface-800 border border-slate-700 rounded-xl px-4 py-2 text-sm text-white focus:outline-none focus:ring-1 focus:ring-brand-500 w-full sm:w-64"
            >
              {empresas.map(emp => (
        ...
        {!config ? (
          <div className="text-center py-12 text-slate-400">
            ...
            <p>No se encontró configuración para esta empresa.</p>
          </div>
        ) : (
          <>
            {/* ── Aviso de seguridad ─────────────────────────── */}
            <div className="bg-amber-500/5 border border-amber-500/20 rounded-2xl px-4 py-3 flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5 leading-relaxed">
                  Los valores aquí almacenados son visibles solo para administradores de la empresa y Super Admins.
                  Las claves API se guardan en la base de datos segura con RLS.
                </p>
              </div>
            </div>
            ...
            >
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <FieldInput id="portal_url" label="URL de retorno para el cliente"
                  value={config.portal_url || ''} onChange={set('portal_url')}
                  placeholder="https://iconsupport.vercel.app" 
                  helpText="URL a donde el cliente será redirigido o linkeado por fuera del portal si lo deseas." />
              </div>
              ...
              <FieldInput id="empresa_logo_url" label="URL del logotipo (PNG/SVG)"
                ...
                placeholder="https://..." helpText="Usado en el portal público, reportes PDF y emails al cliente." />
            ...
            >
              ...
              <SecretInput id="gemini_api_key" label="Gemini API Key"
                ...
                placeholder="AIzaSy..."
                helpText="Clave de la API de Google Gemini para el bot de IA." />
            ...
            >
              <div>
                ...
                <textarea
                  ...
                  placeholder="¡Hola! 👋 Bienvenido a nuestro soporte..."
                />
              </div>
              ...
              <div className="flex flex-col md:flex-row gap-6 items-start">
                <div className="bg-surface-800 p-4 rounded-2xl flex items-center justify-center border border-slate-700">
                  <div className="bg-white p-3 rounded-xl">
                    <img src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent((import.meta.env.VITE_APP_URL || window.location.origin) + `/solicitar-soporte?empresa=${selectedEmpresaId}`)}`} alt="QR Portal" className="w-32 h-32" />
                  </div>
                </div>
                <div className="flex-1 space-y-4">
                  <p className="text-sm text-slate-300 leading-relaxed">
                    Comparte este código QR o el siguiente enlace con tus clientes para que puedan generar tickets de soporte técnico asociados a tu empresa sin necesidad de iniciar sesión.
                  </p>
                  ...
                  <div className="bg-surface-800 border border-slate-700 rounded-xl p-3 flex flex-col gap-2">
                    ...
                    <div className="flex items-center gap-2">
                      <code className="flex-1 text-xs text-brand-400 font-mono bg-surface-900 py-2 px-3 rounded-lg overflow-x-auto whitespace-nowrap">
                        {(import.meta.env.VITE_APP_URL || window.location.origin) + `/solicitar-soporte?empresa=${selectedEmpresaId}`}
                      </code>
                      <button
                        onClick={() => {
                          navigator.clipboard.writeText((import.meta.env.VITE_APP_URL || window.location.origin) + `/solicitar-soporte?empresa=${selectedEmpresaId}`)
                      ...
                      <a
                        href={`/solicitar-soporte?empresa=${selectedEmpresaId}`}
                        target="_blank"
            ...
            >
              <ol className="space-y-3 text-sm text-slate-400">
                {[
                  { n: 1, title: 'Aviso sobre SaaS', desc: 'Nota: En este sistema Multi-Tenant, el webhook de WhatsApp es único globalmente. Asegúrate de configurar un solo número maestro global o configurar webhooks múltiples.' },
                  { n: 2, title: 'Prueba enviando un mensaje', desc: 'Escribe "hola" al número de WhatsApp Business y el sistema detectará el Phone ID asociado a tu empresa.' },
                ].map(({ n, title, desc }) => (
